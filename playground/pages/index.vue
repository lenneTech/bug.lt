      ...
      <div class="space-y-8">
        ...
        <UCard>
          ...
          <div class="space-y-2 text-sm">
            <p><strong>Button Position:</strong> {{ position }}</p>
            <p><strong>Auto Show:</strong> {{ runtimeConfig.public.bugLt?.autoShow ? 'Yes' : 'No' }}</p>
          </div>
        </UCard>
...
const testInput = ref('')
const counter = ref(0)
const position = computed(() => runtimeConfig.public.bugLt?.position || 'bottom-right')

// Demo functions
const logInfo = () => {
  console.info('This is an info message from the demo', { timestamp: new Date() })
  toast.add({ title: 'Info logged', description: 'Check the console logs' })
}

const logWarning = () => {
  console.warn('This is a warning message', { component: 'DemoPage' })
  toast.add({ title: 'Warning logged', color: 'amber' })
}

const logError = () => {
  console.error('This is an error message', new Error('Demo error'))
  toast.add({ title: 'Error logged', color: 'red' })
}
